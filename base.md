...
### Оператор Элвиса
Оператор `?:` - бинарный оператор, который возвращает первый операнд, если он истинен, и второй в ином случае.

Сигнатура:

    $result = $condition ?: $valueIfFalse;

Пример отображения имени или никнейма: 

    function greeting($name, $nickname)
    {
        $user = $name ?: $nickname;
        return $user;
    }

    echo greeting('', 'oshu'); // oshu
